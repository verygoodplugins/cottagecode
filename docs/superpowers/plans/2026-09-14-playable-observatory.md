...
- [ ] Add pure behavior tests and implement feed/PR/interior modules in parallel.
- [ ] Integrate the data contract into the current map and demo; preserve last snapshot on error.
- [ ] Add stable world positions, movement/collision, roof transition, room interaction, journal, and PR dispatch stands/filtering.
- [ ] Add pond/wildlife interaction, opt-in proximity audio, apprenticeship, explicit roads/couriers.
- [ ] Add follow bench, since-visit noticeboard, persisted milestone scrapbook and bounded recorded replay.
- [ ] Document contract, usage, configuration, controls, and limitations.
- [ ] Run node --test, syntax checks, real HTTP smoke and browser journeys on demo and live.
- [ ] Inspect responsive/reduced-motion/keyboard paths; resolve failures before completion.
